...
namespace TravellerFactionSystem
{
    public class TravellerFaction
    {
        ...
        public List<TravellerFactionPersonAsset> FactionMembers { get; set; } = new();
            ...
            List<TravellerPlanetaryBaseAsset> baseLocations = null)
        {
            ...
            if (string.IsNullOrEmpty(factionHeadName))
                factionHeadName = GetNames(1, TravellerNameService.GetNationalitiesNameList(supportingNationality))
                    .First();
            ...
            if (FactionMembers == null || FactionMembers.Count == 0)
            {
                ...
                var factionMemberCount = _randomGenerator.Next(0, 11);
                for (var i = 0; i < factionMemberCount; i++)
                {
                    var memberName = GetNames(1, TravellerNameService.GetNationalitiesNameList(supportingNationality))
                        .First();
